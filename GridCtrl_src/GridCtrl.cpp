...
// erase the background normally through OnEraseBkgnd, and didn't fill the CMemoryDC's
...
BOOL CGridCtrl::SetItemText(int nRow, int nCol, LPCTSTR str)
{
	...
	//if (0 == (int)(m_mygrid.size()))
	//{
	//	return TRUE;
	//}
	//else
	//{
	//	list <GridItems *> ::iterator sortit = m_mygrid.begin();
	//	for (int i = 0; i < ((nRow * GetColumnCount()) + nCol); i++)
	//	{
	//		sortit++;
	//	}
	//	//sortit--;

	//	GridItems *tmp = *(sortit);
	//	tmp->m_data = str;
	//	tmp->GetmyProvider()->NotifyTextChanged();
	//}
...
CImageList* CGridCtrl::CreateDragImage(CPoint *pHotSpot)
{
    ...
    *pHotSpot = rect.BottomRight(); 
    ...
    CBitmap* pOldBitmap = CMemoryDC.SelectObject(&bm);
    CMemoryDC.SetWindowOrg(0, 0);
    
    // Draw cell onto bitmap in CMemoryDC
    CGridCellBase* pCell = GetCell(cell.row, cell.col);
    if (pCell)
        pCell->Draw(&CMemoryDC, cell.row, cell.col, rect, FALSE);
    
    // Clean up
    CMemoryDC.SelectObject(pOldBitmap);
    ReleaseDC(pDC);
...
HRESULT CGridCtrl::OnObj(WPARAM wParam, LPARAM lParam)
{
	if (static_cast<long>(lParam) == static_cast<long>(UiaRootObjectId))
	{
		LRESULT lresult = UiaReturnRawElementProvider(m_hWnd, wParam, lParam, GetGridProvider());
		return lresult;
	}
	return S_OK;
}

CGridCtrlProvider * CGridCtrl::GetGridProvider()
{
	if (NULL == m_pGridProvider)
	{
		m_pGridProvider = new CGridCtrlProvider(this);
	}
	return m_pGridProvider;
}

void CGridCtrl::RefreshMyGridIterms()
{
	while (0 != (int)(m_mygrid.size()))
	{
		list <GridItems *> ::iterator sortit = m_mygrid.begin();
		GridItems *tmp = *sortit;
		tmp->GetmyProvider()->NotifyItemRemoved();
		m_mygrid.erase(sortit);
		delete tmp;
		tmp = NULL;
	}
	m_mygrid.clear();
	...
	int irawctn = GetRowCount();
	int icolctn = GetColumnCount();

	for (int i = 0; i < irawctn; i++)
	{
		for (int j = 0; j < icolctn; j++)
		{
			GridItems *tmp = new GridItems();
			tmp->m_raw = i;
			tmp->m_col = j;
			tmp->m_index = ((i * icolctn) + j);
			tmp->m_data = GetItemText(i, j);
			tmp->m_gridcontrl = this;
			tmp->m_griditemprovider = NULL;
			tmp->GetmyProvider()->NotifyItemAdded();
			m_mygrid.push_back(tmp);
		}
	}
}


//void CGridCtrl::AddGridItems(GridItems *mygrid)
//{
//	m_mygrid.push_back(mygrid);
//	GridItemsProvider *tmp = mygrid->GetmyProvider();
//	tmp->NotifyItemAdded();
//	RefreshGridIterms();
//}
//
//void CGridCtrl::DeleteMyGridItems(int iIndex)
//{
//	list <GridItems *> ::iterator needdelete = m_mygrid.begin();
//
...
//	}
//
//	GridItems *tmp = *(needdelete);
//	GridItemsProvider *tmpprovider = tmp->GetmyProvider();
//	tmpprovider->NotifyItemRemoved();
//
//	delete tmp;
//	tmp = NULL;
//	RefreshGridIterms();
...
void CGridCtrl::AddMyGridItemsRow(int irow)
{
	//for (list < GridItems *> ::iterator it = m_mygrid.begin(); it != m_mygrid.end(); ++ it)
	//{
	//	GridItems *tmp = *it;
	//	tmp->m_index = (tmp->m_raw * GetColumnCount() + tmp->m_col);
	//}
	//m_mygrid.insert()

	//list < GridItems *> ::iterator it = m_mygrid.end();
 //   GridItems *tmp = *(it--);
	//int oldrow = tmp->m_raw;

}

void CGridCtrl::AddMyGridItemsCol(int icol)
{

}

void CGridCtrl::DeleteMyGridItemsRow(int irow)
{

}

void CGridCtrl::DeleteMyGridItemsCol(int icol)
{
