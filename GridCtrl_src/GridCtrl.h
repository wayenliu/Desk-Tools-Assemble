...
public:
	...
	//add by liuwei
public:
	HRESULT OnObj(WPARAM wParam, LPARAM lParam);
private:
	CGridCtrlProvider *m_pGridProvider;
public:
	CGridCtrlProvider *GetGridProvider();
	list <GridItems*> m_mygrid;
	void RefreshMyGridIterms();
	void AddMyGridItemsRow(int irow);
	void AddMyGridItemsCol(int icol);
	void DeleteMyGridItemsRow(int irow);
	void DeleteMyGridItemsCol(int icol);
